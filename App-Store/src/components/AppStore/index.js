...
class AppStore extends Component {
  ...
  render() {
    ...
    const filteredAppsList = appsList.filter(
      eachApp => eachApp.category === selectedTabId,
    )
    const searchResults = filteredAppsList.filter(eachApp => {
      const {appName} = eachApp
      return appName.toLowerCase().includes(searchInput.toLowerCase())
    })
    return (
        ...
        <div className="container">
          ...
          <ul className="apps-container">
            {searchResults.map(eachApp => (
              <AppItem appObj={eachApp} key={eachApp.appId} />
            ))}
          </ul>
        </div>
      </div>
