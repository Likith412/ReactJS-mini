...
class Events extends Component {
  ...
  render() {
    ...
    const selectedEventObj = eventsList.find(
      eachItem => eachItem.id === selectedEventId,
    )
    let registrationStatus = 'INITIAL'
    if (selectedEventObj !== undefined) {
      registrationStatus = selectedEventObj.registrationStatus
    }
          ...
          <ul className="events-list-container">
            {eventsList.map(eachItem => (
              <EventItem
                ...
                isSelected={eachItem.id === selectedEventId}
              />
            ))}
          </ul>
        </div>
